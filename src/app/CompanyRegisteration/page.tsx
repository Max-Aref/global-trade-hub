...
function CompanyRegistration() {
  ...
  const validate = (): boolean => {
    ...
    if (!formData.industryCategory)      newErrors.industryCategory = "Industry is required";
      ...
      <div className="container mx-auto px-4 py-12">
        <div className="max-w-4xl mx-auto">
                ...
                <React.Fragment key={step.label}>
                  <div className="flex flex-col items-center">
                    <div className={`h-10 w-10 rounded-full flex items-center justify-center text-sm font-medium ${
                      step.done ? "bg-brand-500 text-white" : i === 1 ? "bg-brand-500 text-white" : "bg-brand-500/20 border border-white/15 text-white/50"
                    }`}>
                      {step.done ? <FaCheck /> : i + 1}
                    </div>
                    <span className="text-xs mt-1 text-white/60">{step.label}</span>
                  </div>
                  {i < arr.length - 1 && (
                    <div className="flex-1 h-1 mx-2 bg-white/10">
                      <div className={`h-full bg-brand-500 transition-all ${step.done ? "w-full" : "w-0"}`} />
                    </div>
                  )}
                </React.Fragment>
              ))}
          ...
          <div className="border border-white/10 rounded-2xl p-8 bg-gradient-to-br from-brand-950 to-black shadow-[0_0_25px_rgba(140,69,255,0.15)]">
            <h1 className="text-2xl font-semibold mb-1">Company Registration</h1>
            ...
            <form onSubmit={handleSubmit} className="space-y-10">
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <Select
                      ...
                      required
                      options={[
                        { value: "Manufacturer", label: "Manufacturer — مصنّع" },
                        { value: "Wholesaler",   label: "Wholesaler — تاجر جملة" },
                    ...
                    <Select
                      ...
                      options={[
                        { value: "Textiles",      label: "Textiles & Apparel — منسوجات" },
                        { value: "Food",          label: "Food & Beverages — أغذية" },
                        { value: "Handicrafts",   label: "Handicrafts — حرف يدوية" },
                        { value: "Furniture",     label: "Furniture & Home — أثاث" },
                        { value: "Industrial",    label: "Industrial Products — صناعات" },
                        { value: "Chemicals",     label: "Chemicals — كيماويات" },
                        { value: "Agricultural",  label: "Agricultural — زراعة" },
                        { value: "Building",      label: "Building Materials — مواد بناء" },
                        { value: "Medical",       label: "Medical Equipment — طبي" },
                        { value: "Packaging",     label: "Packaging & Printing — تغليف" },
                      ]}
                    />
                  </div>
              ...
              <section>
                ...
                <div className="space-y-4">
                  <div>
                    <label htmlFor="shortBio" className="block text-sm font-medium mb-1.5 text-white/90">
                      Short Bio <span className="text-error ml-0.5">*</span>
                      <span className="text-white/40 text-xs ml-2">(max 250 chars)</span>
                    </label>
                    <textarea
                      id="shortBio"
                      ...
                      className={`w-full bg-brand-950 border rounded-lg px-4 py-3 text-sm text-white placeholder:text-white/30 focus:outline-none focus:ring-2 focus:ring-brand-500 transition-colors resize-none ${
                        errors.shortBio ? "border-error" : "border-white/15 focus:border-brand-500"
                      }`}
                    />
                  ...
                  <div>
                    <label htmlFor="fullDescription" className="block text-sm font-medium mb-1.5 text-white/90">
                      Full Description
                      <span className="text-white/40 text-xs ml-2">(max 1000 chars)</span>
                    </label>
                    <textarea
                      id="fullDescription"
              ...
              <section>
                ...
                <div className="space-y-6">
                  {/* Logo upload */}
                  <div>
                    <p className="text-sm font-medium mb-2 text-white/90">Company Logo</p>
                    <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                      <div className="w-20 h-20 bg-brand-950 border border-white/15 rounded-xl flex items-center justify-center overflow-hidden shrink-0">
                        {formData.companyLogo ? (
                          ...
                          <span className="text-white/20 text-xs text-center px-2">No logo</span>
                        )}
                      </div>
                      <div className="flex-1">
                        <label className="cursor-pointer block">
                          ...
                          <input
                            type="file" id="companyLogo" accept="image/*"
                  ...
                  <div>
                    ...
                    <label className="cursor-pointer block w-fit">
                      ...
                      <input
                        type="file" id="businessLicense" accept=".pdf,.doc,.docx"
                        onChange={handleFileUpload} className="sr-only"
                      />
                    </label>
                    <p className="text-xs text-white/40 mt-1.5">PDF, DOC — max 5 MB</p>
                  </div>
                </div>
              </section>
              ...
              <div className="flex items-start gap-3">
                ...
                />
                <div>
                  <label htmlFor="agreeTerms" className="text-sm text-white/70 cursor-pointer">
                    I agree to the{" "}
                  ...
                  {errors.agreeTerms && (
                    <p className="text-xs text-error mt-0.5">{errors.agreeTerms}</p>
                  )}
                </div>
              </div>
